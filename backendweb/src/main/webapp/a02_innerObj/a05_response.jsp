...
4. 리다이렉트
    ...
    2) 서버 안에서 명령문으로 특정 페이지 이동 처리
        response.sendRedirect(String location)
        요청에 의해서 데이터를 전송하고 그 전송된 내용에 판단에 의해서 페이지가 결정 될 때
        ex) login의 경우, DB 내용에 따라 페이지 결정
...
a09_login.jsp
아이디/패스워드입력
==> himan/7777 일 때는 a10_main.jsp 로그인성공 메인페이지
로그인 실패 a11_logfailure.jsp 이동처리
로그인페이지 이동 클릭 시, a09_login.jsp
 --%>
<body>
...
<div class="container">
	...
	<h3 onclick = "goPage()">location.href로 client단에서 바로 이동</h3>
   ...
   <table class="table table-hover table-striped">
    ...
    <tbody>
    <%
    // 헤더 정보가 저장된 곳(client), 서버에서 이 정보를 받아서 확인 ==> request로 처리
    // 다시 이 헤더 정보를 변경하려면 response 객체를 통해서 속성을 변경하여 전달
    //    ==> 위 나열된 response.XXXX()메서드를 통해서 처리
    // 
