...
1. 요청값이 많은 경우, useBean에서는 자동 setProperty 설정 기능을 제공한다.

2. 단, 자동 설정기능의 경우, 반드시 요청값과 setProperty의 이름과 type이 동일할 때, 가능하다.

3. 기본 형식
    1) 페이지?key1=val1&key2=val2...
    2) 위 해당하는 setXXXX 형식으로 
        요청 key에 해당하는 이름과 할당할 수 있는 type의 데이터를 메서드로 선언할 클래스 선언
    3) <jsp:useBean 선언
       <jsp:setProperty property="*" name="bean아이디"
       이렇게 property의 속성값으로 *를 설정하는 위 query string 요청명과 property가 같고
       type을 할당할 수 있으면 추가 선언하지 않더라도 할당이 된다.
