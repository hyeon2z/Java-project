...
<!DOCTYPE html>
<%--
# 비동기 방식으로 서버의 데이터 가져오기
1. xhr의 비동기 방식이란?
    서버에 요청하고, 다른 기능을 처리할 수 있되, 그 요청에 대한 처리는 이벤트핸들러
    onreadyStatechange에 맞기고 하는 것을 말한다.
    
2. 기능 처리를 위한 핵심 코드
    1) xhr.open(get/post, url, true); 
        마지막 매개변수를 true로 비동기 방식으로 설정 한다.
    ...
    2) 전담 이벤트 핸들러에게 맞김
        xhr.onreadystatechange=function(){
            ...
            xhr.status : http응답 코드 200~299(정상적으로 받음)
                ...
                그 외 에러코드가 일때 처리하지 않는다.
            xhr.responseText : 이 핸들러 메서드 안에서 비 동기적으로 받은 데이터를 처리한다.
            ==> 실제 조건처리 코드
            if(xhr.readyState==4 && xhr.status==200){
            
                $("대상객체선택자").text(xhr.responseText); 서버에서 받은 결과값 출력
            }
        }
