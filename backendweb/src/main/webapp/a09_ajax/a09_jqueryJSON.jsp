...
<%--
# ajax에서 json 데이터의 처리
    1. 웹 프로그램에서 데이터 전송의 속도 이슈로 개발자들이 많은 고민을 하고 처리해왔다.
    2. 초창기에는 text를 그대로 전송해서 처리하거나,
    3. text의 데이터를 구분자를 두어 split()메서드를 이용해서 처리하거나
    4. html을 코드로 전송해서 화면에 입력하는 형식을 처리
    5. xml 형식을 만들어 처리하였다.
    6. 위 같은 방식으로 효과적으로 데이터를 비동기적으로 가져왔는데,
        서버와 클라이언트 간의 네트워크 부하를 초래하는 등 많은 문제를 야기 시켰다.
    ...
    11. 형식
        ...
        네트워크상에 전송할 수 있는 json형식
        ex) {"name":"홍길동", "age":25, "isAult":true}
        
    12. 위 형식으로 문자열 전송을 하면, 클라이언트에서 문자열을 객체화하는 eval()이나,
        JSON.parse()를 활용한다.
        jquery에서는 dataType:"json"을 설정하면 자동으로 객체로 변환되어
        success:function(){}로 받아들여서 처리한다.
        마지막 객체로 받은 내용을 DOM객체를 호출하여 append(), html(), text()
		...
		$("#regBtn, h3").click(function(){
			$.ajax({
				...
				success:function(product){
					...
					var add ="<tr><td>"+product.name+"</td><td>"
					+product.price+"</td><td>"+product.cnt+"</td><td>"+product.tot+"</td></tr>"
					$("tbody").append(add)
					// $("#prodList tbody").append(add)
...
<div class="container">
	<form id="frm01" class="form"  method="post">
  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	    <input name="name" class="form-control mr-sm-2" placeholder="물건명" />
	    <input name="price" class="form-control mr-sm-2" placeholder="가격" />
	    <input name="cnt" class="form-control mr-sm-2" placeholder="갯수" />
	    <button id="regBtn" class="btn btn-info" type="button">물건정보등록</button>
