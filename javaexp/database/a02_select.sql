...
FROM emp;
-- 주의) 데이터베이스에서는 +를 통해 문자열 연결하지 않고, ||를 통해 처리된다.
...
SELECT ename 사원명, sal 급여, sal*3 급여의3배
FROM emp;
...
-- ex3) 사원명-부서번호, 사원번호-사원명, 입사일을 출력
SELECT ename||'-'||DEPTNO "사원명-부서번호", empno||'-'||ENAME "사원번호-사원명", HIREDATE 입사일
FROM emp;

-- ex4) @@@(사원명)의 급여는 @@@(급여)이고, 부서번호는 @@(부서번호)입니다.
SELECT ename||'의 급여는 '||sal||' 이고, 부서번호는 '||deptno||'입니다.' 정보
FROM emp;
/* 
1. 동일한 데이터의 중복 제거
 */
