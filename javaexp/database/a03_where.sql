...
3. 기본 형식
    ...
    2) 논리연산식
        ...
        지원하는 연산 키워드 in, between, exists 등 사용된다.
 */
-- 숫자형 데이터 비교
-- 연봉이 2000이상인 사원정보 출력
SELECT *
...
-- ex2) 직책(job)이 'CLERK'인 사원번호를 직책명, 사원명을 출력하세요
SELECT job 직책, empno 사원번호, ename 사원명
FROM EMP e 
WHERE job = 'CLERK';
...
-- 급여가 5000이상이거나 관리자번호가 7782인 경우 사원정보
SELECT *
FROM EMP
WHERE sal >= 5000
OR MGR = 7782;

-- ex1) 급여가 2000이상이고 직책이 MANAGER인 데이터를 출력
SELECT *
FROM EMP e 
WHERE sal >= 2000
AND JOB = 'MANAGER'
