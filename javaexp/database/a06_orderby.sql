...
2. 데이터의 출력 순서를 특정 커럼을 기준으로 오름차순/내림차순으로 정렬하는 경우가 발생한다.
3. 여러 개의 컬럼에 대해 정렬 순서를 지정해서 처리해야 할 경우
    1차로 컬럼명 정렬옵션, 2차로 컬럼명 정렬옵션으로 오름차순/내림차순으로 처리한다.
4. 기본적인 정렬 방법
    1) 문자값은 알파벳순으로 출력되고, 한글은 가나다순으로 출력된다(오름차순)
...
FROM EMP e 
ORDER BY EMPNO DESC;
-- ex1) 연봉이 가장 높은 사람으로부터 가장 낮은사람 순으로 급여야 이름을 추력하세요
SELECT SAL, ENAME 
FROM EMP e 
ORDER BY SAL DESC;
-- ex2) 입사일을 기준으로 최근에 입사한 사람으로부터 가장 먼저 입사한 사람 순으로 입사일과 사원명을 출력하세요
