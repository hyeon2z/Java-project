...
1. 오라클에서 유일한 식별자의 데이터를 입력하기 위해서
	자동으로 numbering하는 객체가 있는데, 이것을 sequence라고 한다.

2. 활용 예
    1) 주로 고유의 numbering을 이용해서, 데이터를 가져오거나
        특정한 경우에 데이터를 입력(primary key)시 유일 식별자를 처리할 때 사용한다.
        주로 등록 시 활용하는 경우가 많다.
        
    2) 기본 형식
        create sequence 시퀀스명;
            기본적으로 1부터 1씩 무한대로 증가 시키는 시퀀스가 만들어 진다.
        create sequence 시퀀스명
            increment by 증가단위
            start with 시작번호
            minvalue 최소값
...
-- 위와 같이 고정된 문자를 조합해서 학번을 설정할 수 있다.
-- ex) 제품정보테이블(제품번호(고정7자리),제품명,가격) 있을 때, 제약조건을
-- 설정해서 테이블을 만들고, sequence와 조합해서 고유 제품번호를 입력 처리
CREATE SEQUENCE seq03
    START WITH 1000000;
CREATE TABLE proinfo02(
	prono char(7) PRIMARY KEY '제품번호',
	protype varchar2(50) NOT NULL '제품명',
	proprice number(30) NOT NULL '가격'
);
CREATE SEQUENCE prod_seq
	START WITH 1000;
INSERT INTO proinfo01
values(
'PRO'||prod_seq.nextval, '오렌지',1500);
SELECT * FROM proinfo01;
