...
2. 관계형 데이터베이스 분야의 표준으로 자리잡혀서, 여러 테이블과의 조인 관계에서 사용하여,
    테이블을 효율적으로 관리하고, 데이터를 조회 처리할 때 사용된다.
    
3. 단계별 학습 내용
    1) 기본 조인구문 형식 이해 및 활용
    2) 여러가지 조인구문 형식 활용
    3) subquery, inline, view를 통한 조인 구문 처리
        - 주로 그룹함수 처리 후 조인처리
    4) 여러가지 복합 형식의 조인구문 처리
...
1. 테이블과 테이블의 연관관계를 아래와 같이 설정한다.
    ...
    - 이 내용을 조인의 가장 기본형식인 두 개 테이블간의 공통컬럼을 기준으로
    조인하는 inner 조인의 형식이다.
...
FROM emp e, dept d
WHERE e.deptno = d.deptno;
-- ex) 조인 구문을 활용하여 부서명, 사원명, 부서위치, 급여를 출력
SELECT dname, ename, loc, SAL 
FROM emp e, dept d
WHERE e.deptno = d.deptno;
-- ex2) employee, locations 테이블에 공통컬럼을 확인하여
         -- 조인하여 데이터를 출력하세요.
...
/*
# 조인 구문의 결과에 의한 조건 및 함수처리
1. 위와 같이 조인구문에 의해 처리된 결과는 다시, 하나의 데이터의 결과로
    다시 이것을 이용해서 조건/그룹함수/일반함수를 적용하여
    데이터를 처리해 결과를 출력할 수 있다.

*/
-- 부서명에 A가 포함되는 사원명, 부서명을 출력
SELECT ename, dname
FROM emp e, dept d
WHERE e.deptno = d.deptno
AND dname LIKE '%A%';
...
-- ex2) employees, jobs 테이블을 이용하여
--    직책명별(job_title)로 최고급여를 출력
