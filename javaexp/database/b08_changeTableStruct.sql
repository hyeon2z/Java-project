/*
# 테이블의 구조 변경
1. 테이블을 사용하다 보면 여러가지 상황에 따라 테이블의 구조를 변경하는 경우가 발생한다.
    테이블의 구조를 변경하는데 필요한 명령어를 파악하고,
    테이블 구조의 변경 시 발생하는 데이터 처리에 대한 내용을 파악해보자.
...
-- 해당 테이블에 bonus 컬럼을 추가하고, default데이터 0을 입력처리하세요
CREATE TABLE emp08
...
3. 주의사항
    1) 2개 이상의 컬럼이 존재하는 테이블에서만 삭제 가능
        하나의 컬럼인곳의 삭제는 테이블에 대한 의미가 없기에 삭제 불가
    2) 한번에 하나의 컬럼만 삭제 가능
*/
SELECT * FROM emp02;
ALTER TABLE emp02
DROP COLUMN ename;
-- ex) emp 테이블을 이용해서 emp08테이블을 생성하고,
-- 해당테이블에 dname, loc 컬럼 추가, deptno 컬럼 삭제 처리하세요
CREATE TABLE emp09
AS SELECT *
FROM emp;
...
3. 변경 시 주의사항
    ...
    2) 기존 데이터가 있는 경우
        - 타입 변경은 char/varchar2만 허용된다.
        - 변경한 컬럼의 크기가 저장된 데이터의 크기보다 같거나 클 경우 변경가능
...
SELECT * FROM emp10;
-- # 데이터가 있을 때
-- 0. 다른 유형으로 변경하는 것은 불가능(문자형==> 숫자, 숫자==> 날짜)
...
SELECT *
FROM emp12_new;
RENAME emp09_new TO emp12_new;
SELECT *
FROM emp09_new;
alter TABLE emp12_new
RENAME COLUMN ename TO name;
-- ex) emp02 테이블을 emp02_cpy 로 변경하고,
-- empno ==> no, ename ==> name, sal ==> salary 변경처리하세요
DROP TABLE emp02_cpy;
CREATE TABLE emp02
AS SELECT * FROM emp;
