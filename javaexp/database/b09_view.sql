...
4. 뷰의 한계
    1) 제한적 사용 : 무결성 제약조건, 표현식, group by 유무에 따라 dml명령 등은 제한적으로 사용된다.
    2) 수행불가능 : DISTINCT, 그룹함수, GROUP BY, START WITH, CONNECT BY,
        ROWNUM은 포함 할 수 없다.
...
5. 뷰 생성 기본
    ...
    2) 옵션
        or replace : 뷰를 생성할 때, 생성된 뷰가 있으면 동일한 이름의 뷰를 재성성
...
AS SELECT count(*) 사원수, max(sal) 최대급여, min(sal) 최소급여, avg(sal) 평균급여
FROM emp
GROUP BY deptno;
...
-- ex) 위 emp 테이블에 사원이 추가되고 데이터가 추가 되면 통계치 변경이 되기 때문에 그 내용이 반영된다.
SELECT *
FROM v_emp_statics;
...
1. 단순 뷰
    하나의 테이블로 생성
    그룹 함수의 사용이 불가능 - 실제 테이블의 데이터가 아니기에
    distinct 사용할 때, 불가능
...
SELECT * FROM dept13;
-- 복합뷰는 여러 컬럼의 데이터를 통해 복합적으로 만들었기에 입력이 되지 않는다.
