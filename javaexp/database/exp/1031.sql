...
INSERT INTO emp05 values(7935, 'TOM', 'CLERK', 7788, sysdate, 2500, 0, 15);

--[1단계:확인] 6. emp05 테이블에서 사원의 이름이 "SMITH"인 사원의 정보를 수정합니다.
UPDATE emp05 SET ename = 'HTIMS'
WHERE ename = 'SMITH';
...
SELECT upper(ename) 대문자, length(ename) 길이
FROM emp05;

--[1단계:확인] 13. 모든 사원의 이름의 첫 글자만 대문자로 변환한 후, 마지막 문자를 출력하세요.
SELECT ename, INITCAP(ename) 대문자, substr(ename,-1) 마지막
FROM emp05;

--[1단계:확인] 14. 모든 사원의 이름의 첫 글자를 제외하고 모두 소문자로 변환하세요.
SELECT ename, lower(substr(ename, 2, 6)) "첫글자제외 소문자"
FROM emp05;

--[1단계:확인] 15. `DEPTNO`를 문자열로 변환한 후, 앞에 0을 5개 채워 출력하세요.
SELECT lpad(deptno,7,0)
FROM emp05;
...
SELECT lower(ename) 소문자, lengthb(ename) 바이트길이
FROM emp05;

--[1단계:확인] 18. 사원의 이름을 모두 대문자로 변환한 후, 뒤에 `!!!`를 붙여 출력하세요.
SELECT concat(upper(ename), '!!!') 대문자
FROM emp05;

--[1단계:확인] 19. 사원의 이름에서 첫 번째 문자와 마지막 문자를 출력하세요.
SELECT ename, substr(ename, 1, 1) 첫번째, substr(ename, -1, 1) 마지막
FROM emp05;

--[1단계:확인] 20. 사원의 이름 중 앞에 'S'로 시작하는 사원을 찾아 이름을 대문자로 출력하세요.
SELECT Upper(dname) 대문자출력
FROM dept03
WHERE dname LIKE 'S%';

--[1단계:확인] 21. 사원의 이름과 직업을 `/`로 연결한 후, 앞뒤에 `###`를 추가하여 출력하세요.
SELECT rpad(lpad(concat(concat(ename,'/'),job),LENGTH(ename)+LENGTH(job)+4,'#'),LENGTH(ename)+LENGTH(job)+7,'#') show
FROM emp05;

--[1단계:확인] 22. 시원의 이름에서 처음으로 'E' 문자가 나오는 위치와 마지막 글자를 연결하여 출력하세요. 
SELECT concat(instr(dname,'E'), substr(dname,-1,1)) show
FROM dept03;

--[1단계:확인] 24. 사원의 이름에서 오른쪽 공백을 제거한 후, 그 결과값의 길이를 출력하세요
SELECT LENGTH(rtrim(dname, ' '))
FROM dept03;
