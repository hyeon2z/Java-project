...
//		# 객체형태의 메모리 구조
//		 1. 배열부터는 객체로 포함되어 stack영역과 heap영역을 둘다 활용하여 사용한다.
//
//		 2. 즉, heap영역에 실제 객체의 메모리를 사용하고, stack영역에서는 이 heap영역의 주소값을 저장해서 가지고 있다.
//		     ex) 도서관에서 도서를 찾는 단말기에서 도서의 위치정보를 가지고 있고,
//		          실제 이 위치정보를 찾아가면 도서관에 특정한 곳에 책이 있는것에 비유할 수 있다.
//		 
//
//		 3. 객체의 선언과 할당.
...
//		     2) 배열선언과 할당.
//		         배열명 = new 타입[배열의 갯수];
//			 현재 단계까지는 배열에 default 데이터만 할당된 상태 즉, int[]일 경우 0이 각 구성요소에 할당.
...
//			 이와같이 선언과 동시에 0.0이외에 초기값을 할당하는 처리를 한번에 할 수 있다.
//			 arry : heap영역 주소값
//			 arry[0] : 첫번째 데이터 0.7 할당되어 있음
		
		int intArry[]; // 배열 선언
		intArry = new int[5]; // 배열의 초기값 할당된 heap영역 선언
