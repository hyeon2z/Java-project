...
public class A01_Basic {
	...
	# 객체 지향 프로그램
 		...
 		3. 객체의 상호작용
     		- 객체들은 서로간의 기능(동작)을 이용해서 데이터를 주고 받는 기능을 처리한다.
    ...
    # 객체와 클래스
 		1. 현실세계 비유
     		현실세계 : 설계도 ==> 구체적인 사물/객체
         		ex) 롯데월드 타워의 설계도를 통해서 롯데월드 타워 건축
 		...
 		3. 클래스로부터 만들어진 객체를 해당 클래스의 인스턴스(instance)
     		instance : 참조 - heap영역의 주소를 참조해서 변수를 만들었기 때문에 참조변수라고 한다.
         		ex) 하나의 클래스를 기준으로 여러개의 객체가 여러개의 참조변수로 생성 가능하다.
 		...
 		4. 하나의 클래스로부터 여러 개의 인스턴스를 만들 수 있다.
     		하나의 롯데월드타워 도면으로부터 서울, 부산, 제주도에 실제 건물을 건축할 수 있다.
	...
	public static void main(String[] args) {
		...
		Person p01 = new Person();
		System.out.println(p01); // heap 영역에 특정한 주소.
		p01.name = "홍길동";
		p01.age = 25;
		
		Person p02 = new Person();
		System.out.println(p02); // heap 영역에 특정한 다른주소.
		p02.name = "김길동";
		p02.age = 27;
...
// Tower 클래스, loc 속성을 지정하여
// 이것을 통해서 만들어진 t1 t2 t3로 객체를 생성하고, loc에 서울/부산/제주 로 선언하여
// heap영역의 주소값과 각각의 loc을 출력해보세요
