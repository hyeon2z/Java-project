...
public class A05_StringBuffer {
	...
	public static void main(String[] args) {
		/*
		# String 메모리 += 의 위험성
		1. String은 문자열 데이터인데 += 을 통해서 문자열을 추가할 수 있는데,
		기존에 특정한 위치에서 메모리가 확대되는 것이 아니라,
		새로운 주소값을 가지고 다시 메모리를 할당하는 특징이 있다.
		
		2. 반복문안에 += 을 사용하여 문자열을 추가하는 것은
		메모리의 효율에 문제가 발생한다.
		 */
		String name1 = "김길동";
		String name2 = "김길동";
		...
		System.out.println("# 문자열 누적 시 새로운 주소 할당 #");
		...
		데이터가 누적하당하면 heap영역을 계속 만들어 메모리의 overflow가 발생할 수 있다.
		
		# 문자열 데이터를 처리할 때, 문자열이 늘어나더라도 동일한 heap 주소 안에서
		데이터를 처리할 수 있게 처리하여야 하는데, 이것이 StringBuffer()객체이다.
		
		# StringBuffer의 활용
		1. 객체 생성
		    StringBuffer sbf = new StringBuffer();
		    StringBuffer sbf2 = new StringBuffer("초기문자");
		...
		4. 기타 기능 메서드
		    ...
		    reverse() : 문자열 뒤집기 처리
		    setCharAt(int idx, char ch) : 특정 위치에 문자를 대체처리(문자 한 자)
		...
		sb.append("홍"); System.out.println("추가 후 주소값 : " + System.identityHashCode(sb));
		sb.append("길"); System.out.println("추가 후 주소값 : " + System.identityHashCode(sb));
		sb.append("동"); System.out.println("추가 후 주소값 : " + System.identityHashCode(sb));
		sb.append("!"); System.out.println("추가 후 주소값 : " + System.identityHashCode(sb));
		sb.append("~"); System.out.println("추가 후 주소값 : " + System.identityHashCode(sb));
		System.out.println("최종 데이터 : " + sb);
		// ex) StringBuffer로 학생의 번호를 1번을 초기에 넣고,
		// 반복문과 append()를 통해 2~100까지 추가한 후
		// 초기주소값과 마지막 데이터가 다 할당 후 주소값을 확인
		
		StringBuffer st = new StringBuffer("1번");
		System.out.println("초기주소값 : " + System.identityHashCode(st));
		
		for(int i = 2; i <= 100; i ++) {
			st.append(" " + i + "번");
		}
		System.out.println(st);
		System.out.println("할당 후 주소값 : " + System.identityHashCode(st));
