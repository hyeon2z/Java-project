...
import javaexp.a06_object.vo.Emp;
...
//		Database
//		 primary key : 키로 설정한 컬럼은 row단위 해당컬럼이 동일한 경우에 입력자체가 되지 않기에 중복을 막아준다.
//			empno ename job
//			10000 홍길동   대리
//			10001 김길동   사원
//			10002 고길동   부장
//			10000 이길동   사장 (입력자체가 되지 않음. empno가 key일때)
...
2. 배열의 문제점
    ...
    2) 객체 삭제했을 때, 객체를 저장하는 문제 발생
        ...
        ==> 크기가 커지거나 작아지지는 못하는 단점 있음
3. 동적배열의 필요
    1) 객체들을 효율적으로 추가,삭제,검색 할 수 있음
    2) 인터페이스를 통해 정형화된 방법으로 다양한 컬렉션 클래스를 사용 및 확장성 있게 활용가능
...
1. 인터페이스와 실제 객체들
    1) List인터페이스 : 순서를 유지하고 저장, 중복 저장 가능
        ...
        인터페이스를 상속받아서 사용자 정의 실제 클래스를 정의가 가능
        
    2) Set인터페이스 : 순서를 유지하지 않고 저장, 중복 저장 불가능
        HashSet, TreeSet
    
    3) Map인터페이스 : 키와 값의 쌍으로 저장, 키는 중복 저장 안됨
