...
public class A03_FileStream {
	...
	public static void main(String[] args) {
		/*
		# 파일 처리 Stream
		1. 파일을 하드웨어적으로 만들거나, 파일을 소프트웨어적으로 처리하는 객체를 사용할 때, 활용된다.
		
		2. api에 있는 File객체
		    1) 데이터 코드를 통해서 프로그램의 메모리에 할당해서
		        전송/출력 할 수도 있고, 하드웨어적으로 특정한
		        물리적 파일을 생성하고, 저장할 수도 있다.
		    
		    2) 파일 처리 순서
		        - 파일객체 생성 : File f01 = new File("경로와 파일명");
		        - 기능 메서드를 통해 물리적 파일/디렉토리 생성
		            createNewFile()
		    ...
		    3) 지원하는 여러가지 기능메서드
		        ...
		        String[] list() : 경로에 포함된 파일명 또는 서브디렉토리명 리턴
		        File[] listFiles() : 경로에 포함된 파일 및 서브디렉토리 목록 전부를 File 배열로 리턴
		 */
