...
public class A02_primitiveDataType {
	...
	public static void main(String[] args) {
		...
		# 자바의 stack 영역에서 사용되는 기본 데이터 유형
			stack 영역에서는 할당과 동시에 해당 변수의 내용을 가지고 있다.
			ps) 객체의 경우는 stack영역과 heap영역을 사용하는데
			heap영역에 실제 데이터가 할당되고, stack영역에서는 이 heap영역의 주소값을 할당한다.
		1. 숫자형
			1) 정수형
				...
				int:4byte(32bits) **(정수형의 default)
					...
					특히 연산을 하여 할당하는 경우 반드시 int 유형 이상으로 할당하여야 한다.
					byte/short 유형은 연산의 결과값을 할당하지 못한다.
				long:8byte(64bits)
						long num01 = 2147483647214L;
					위 21이상의 int형 범위를 초과할 때는
					반드시 L이라고 붙여야 그 이상의 데이터를 사용할 수 있다.
		...
		2. 문자형(char)
			문자형은 정수형에서 파생된 것으로 2byte로
				정수형 0~65535범위의 코드값을 유니코드값으로 연결하여 처리한 것을 말한다.
				보통 영문은 1byte로 표현을 할수 있고, 한글이나 기타 언어는 2byte범위로 표현한다.
			char ==> char[] ==> String
			문자		 문자배열		문자배열의 type화하여 객체처리
								객체지향프로그램에서는 꼭 필요
								메서드(객체내 함수)를 통해 여러가지 기능을 지원하기 위해 꼭 필요로 함.
								"홍길동 김길동 신길동" ==> 배열
		...
		3. 논리형
			boolean값을 true/false 값을 할당할 수 있는 데이터유형 이다.
			int point = 90;
		 ...
		 */
		byte byte01 = 25;
//		byte byte02 = 225; 범위를 벗어나고 기본 정수형 데이터는 default로 int 형이기에 에러가 발생.
		short short01 = 35;
		// 2byte범위의 데이터를 할당할 때 사용된다.
		// 주의) byte, short 범위라 하더라도 +(연산자)가 들어가는 순간
		// 이것을 할당할 수 있는 범위는 int형 이상이어야 한다.
		byte byte03 = 30;
//		byte sumByte = byte01 + byte03; 범위초과 에러
		...
		// Math.random()
//		int maxNumOver = 214748364723; // int 범위 초과 에러
...
//		int ==> char ==> char[] ==> String
		...
		char letter = 'K';
		// 자바에서 문자형 데이터 유형을 ''사이에 글자 한자 를 할당할 수 있다.
		...
		System.out.println("문자K의 유니코드 값 : " + letterCode);
		// 정수형의 default은 int, 실수형의 dafault는 double로 default 유형이 아닌 경우
		// long은 초과 처리할 때 L을 붙이고, 실수유형의 경우 f를 붙여서 처리한다.
		// # boolean형 데이터 유형
		// true/false
		boolean isTrue = true;
		boolean isFalse = false;
		boolean isOver = largeNumber>=10000;
		// 비교연산자를 이용해서 true/false 값을 가져올 수 있다.
		System.out.println("# boolean 데이터 처리 #");
		System.out.println(isTrue);
		System.out.println(isFalse);
		System.out.println(isOver);
		
		// ex) 정수형 데이터로 점수 80점을 point01변수에 할당하고 boolean 형으로 
		// isPass로 70점 이상일 때 (>=)로 선언하여 합격여부를 true/false로 출력하세요.
