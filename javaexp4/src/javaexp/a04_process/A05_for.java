...
//
//		 3. 기본 구성요소 내용
//		     1) 초기값 : 반복구문에 사용할 변수의 최초값을 설정할 때, 선언한다. 한번만 수행한다.
...
//			 cnt-=2 : 2씩 감소
		...
		for(int cnt = 2; cnt <= 15; cnt += 2) {
			System.out.println("2부터 15까지 짝수로 증가 : " + cnt);
		}
		// ex2) 9부터 2까지 홀수로 감소
		for(int cnt = 9; cnt >= 2; cnt -= 2) {
			System.out.println("9부터 2까지 홀수로 감소 : " + cnt);
		}
				
		// ex3) 구구단의 3단에 1~9까지 곱셈 결과 출력
		for(int cnt = 1; cnt <= 9; cnt += 1) {
			System.out.println("3단 1~9까지 출력 : " + (cnt * 3));
		}
	}
