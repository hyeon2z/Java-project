...
# 메서드
 ...
 3. 메서드의 핵심기능
     ...
     1) 기본 형식
         접근제어자 리턴유형 메서드형(매개변수1, 매개변수2...) {
	     프로세스 처리
	     return 리턴할데이터;
	 }

     2) 구성요소 내용
         리턴유형 : 이 메서드를 통해서 처리하는 결과값을 리턴하는데,
	         메서드 선언의 가장 밑부분에 실제 리턴할 데이터의 유형을 선언하는 것을 말한다.
		 실제 데이터가 5가 리턴되면 리턴 유형은 int이어야 한다.
		 ...
		 매개변수
	       ...
	       메서드명과 매개변수의 선언기준을 외부에서 호출할때 식별할 수 있는 여부에 따라
	       동일한 메서드명으로도 선언이 가능하다. 이것을 메서드 오버로딩이라고 한다.
		     
		 프로세스처리
	       매개변수에 데이터를 받아, 전역변수(필드)에 할당하거나
	       특정 조건/반복 처리를 하는 등 여러가지 알고리즘 처리도 가능하다.
	 
	     최종 리턴값
	       처음에 설정한 리턴유형에 맞는 최종 리턴값을 해당 메서드를 통해 처리할 수 있다.
	       단, return XXX형식으로 선언하지 않을 경우도 있는데
	       이 때는 반드시 return 유형을 void라고 선언하여야 한다.
...
*/
		...
		//class ReturnExp02로 선언하여 정수, 실수, 문자열, boolean 리턴값을 메서드로 선언하고, main()에서 호출하여 출력.
		
		ReturnExp02 re02 = new ReturnExp02();
		System.out.println("정수형 리턴 : " + re02.return01());
		double dbl01 = re02.return02();
		System.out.println("실수형 리턴 : " + dbl01);
		System.out.println("입력받은 문자열 : " + re02.return03());
...
class ReturnExp{
	...
	// String str = "안녕하세요"; ..어떤타입으로 선언할지 정하는것
	String call02() {
		return "안녕하세요";
	}
