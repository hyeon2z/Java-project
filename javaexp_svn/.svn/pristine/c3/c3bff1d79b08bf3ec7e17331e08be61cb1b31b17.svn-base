package javaexp.z02_homework.a12_ljh.a1016;
...
//		[1단계:개념]  1. 예외처리기본 형식과 구체적인 예외 처리 순서 프로세스
		...
		- 예외 처리 순서 프로세스
		위에서 아래로 & 왼쪽에서 오른쪽으로 예외 코드 처리 및 finally(예외 발생 여부와 상관 없이) 처리
		 */
...
//		[1단계:개념]  3. 실행예외와 컴파일예외의 차이점을 기본 예제로 기술하세요.
		...
		/*
	    // [1단계:조별]  9. Scanner로 물건명 가격 갯수를 입력하여 출력하게 할 때, 각 데이터를 입력하지 않을 때, 강제 예외를 발생해서 데이터를 입력하게 처리하세요.
		Scanner sc = new Scanner(System.in);
		String product = null;
		int price = 0;
		int cnt = 0;
		...
		try {
			...
			if(product == null || product == "") {
				throw new Exception("물건명을 입력해주세요.");
			}
			System.out.print("가격 : ");
			String inputPrice = null;
			inputPrice = sc.nextLine();
			if(inputPrice == null || inputPrice == "") {
				throw new Exception("물건의 가격을 다시 입력해주세요.");
			}
			price = Integer.parseInt(inputPrice);
			
			System.out.println("개수 : ");
			String inputCnt = null;
			inputCnt = sc.nextLine();
			if(inputCnt == null || inputCnt == "") {
				throw new Exception("물건의 개수를 다시 입력해주세요.");
			}
			cnt = Integer.parseInt(inputCnt);
...
// ----------------------------------------------------------------
		...
		public void databaseQuery() {
		    try {
		    	// 데이터베이스 처리 시 반드시 필수 예외 처리하여야 한다.
