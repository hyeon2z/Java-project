...
2. 예외가 발생하면 즉시 해당 예외를 처리할 수 있는 가장 가까운 `
catch` 블록으로 제어가 이동됩니다.
...
```java
// 예외 위임을 하지 않더라도 처리가능(런타임 예외)
public void runtimeExceptionExample() {
    int[] arr = new int[5];
    arr[10] = 50;  // ArrayIndexOutOfBoundsException
}
// 예외 위임을 반드시하여야 처리가능(컴파일 예외)
...
이 계층구조를 통해 예외들을 그룹화하고, 공통된 로직을 재사용할 수 있습니다. 예를 들어, 여러 예외들이 같은 부모 예외로부터 상속받는다면, 해당 부모 예외만으로 여러 예외를 함께 처리할 수 있습니다.

### [1단계:개념] 6. 예외의 위임

예외 위임이란 메서드나 생성자에서 발생하는 예외를 그대로 호출한
곳으로 던져서 처리하게 하는 것을 말합니다. `throws` 키워드를 사용하여
 예외를 위임합니다.
...
예외 강제란 특정 조건에서 프로그래머가 의도적으로 예외를 발생시키는 것을
 말합니다. `throw` 키워드를 사용하여 예외를 강제로 발생시킬 수 있습니다.
