...
1. 숫자나 날짜, 문자열 타입은 데이터의 변환에서 중요한 부분을 이루고 있다.
   데이터베이스에서도 원하는 형식으로 타입변환이 함수에 의해서 처리된다.
2. 형변환 유형
	1) 묵시적 데이터 타입 변환
	...
	2) 명시적 데이터 타입 변환(함수를 통해)
	    오라클에서 각 유형마다 변환함수를 선언하여, 명시적 형반환을 처리하고 있다.
	    to_char() :  문자열형으로 변환
	    to_date() :  날짜형으로 변환
	    to_number() : 숫자형으로 변환
 * */
...
1. to_char()를 사용하면 date 타입의 컬럼에서 특정 속성/매개변수를 이용해서
	원하는 형식의 문자열을 출력할 수 있다.
	1) 기본 형식
		to_char(날짜유형데이터, 형식)
	2) 날짜 ==> 문자열로 바꾸어서 출력하는 형식의 매개변수값
		'Q' : 해달 날짜의 분기(1사분기, 2사분기.. 4사분기 표현)
		'MM' : 월을 두가자리로 표현
		'MONTH' : 영문표기
		'WW' : 연단위 주차표기 @@년도의 @@째주차
		'W' : 월단위 주차표기 @@월의 @@째주차
...
SELECT ename, hiredate,
       ...
       TO_CHAR(hiredate,'YYYY"년" MM"월" DD"일" Q"/4분기"') 
       "입사년/월/일/분기"
  FROM emp
...
SELECT ename, sal, to_char(sal,'$99999') "달러표시",
  ...
  FROM emp; 
-- ex)  사원명, 급여, 급여+보너스(천단위 콤마, 소숫점2자리표시) 총계 
--      nvl, to_char 활용
SELECT ename, sal,comm, 
	TO_char( sal+nvl(comm,0),'9,999,999.99' ) "총계"
FROM emp;
/*
...
2. 형식
   ...
   ps) 프로그램에서 입력되는 데이터는 문자열이기 때문에, 숫자형 데이터는
...
/*
# 문자열을 숫자형으로 바꾸어 주는 to_date('날짜형문자열','YYYY MM DD-형식표현')
1. 대부분의 데이터는 날짜형으로 입력불가능하다. 문자열로 입력하여,
	위와 같은 형변환 함수를 통해서 날짜형으로 변환하여 검색하거나, 데이터를
	입력 처리한다.
2. 기본 형식
...
--  날짜타입 hiredate에 입력한 경우
INSERT INTO emp01(ename,hiredate) values('하길동',
	to_date('2022-11-01','YYYY-MM-DD') );
-- ex)emp01 - 사원번호 9999로 사원정보를 입사일날짜(2023/01/01)와 
--           함께 전체 입력해보세요.
INSERT INTO emp01 values(
	9998,'천길동','과장',7566,to_date('2023/01/01 07:23:45',
	                                'YYYY/MM/DD HH:MI:SS'),
	4500,1000,10
);
SELECT * FROM emp01;
