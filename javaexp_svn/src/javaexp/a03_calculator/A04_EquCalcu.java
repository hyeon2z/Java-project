...
//		boolean isValid = id.equals("himan") && 
//							pass.equals("7777");
...
//		boolean isValidPt = pt>=0 && pt<=100;
//		System.out.println("입력된 점수:"+pt);
//		System.out.println("유효범위여부:"+isValidPt);
		/*
		# !(not) : 부정연산자
		1. 논리식을 처리할 때, true이면 false, false이면 true를
			처리해야하는 경우가 있다. 이때, 활용하는 논리연산자가
			부정연산자 !(not) 이다.
		2. 기본적인 비교연산식을 true <=> false로 처리할 뿐아니라.
			!(age>=18) ==> age<18
		3. and, or가 포함된 논리연산자는 !을 통해서 드모르강법칙이
			적용된 내용을 처리해준다.
		...
		System.out.println("유여부:"+!isFree);
		// 1~10까지 임의의 수를 출력하고
		// hint) 점수%2==1 홀수, 점수%3==0 3의배수
		// 1. 홀수이면서 3의 배수인 경우와 
		// 2. 짝수이거나 3의 배수가 아닌 경우를 위 !(not)연산자를
		//    이용해서 출력하세요
		int rNum = (int)(Math.random()*10+1);
		System.out.println("임의의 수:"+rNum);
		boolean isOdd3Dbl = rNum%2==1&&rNum%3==0;
		System.out.println("홀수이면서 3의 배수여부:"+isOdd3Dbl);
		System.out.println("짝수이거나 3의 배수가 아닌 경우:"+
				!isOdd3Dbl);
