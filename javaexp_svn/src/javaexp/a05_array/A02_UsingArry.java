...
public class A02_UsingArry {
	...
	public static void main(String[] args) {
		...
		fruPrices[1] = 4000;
		fruPrices[2] = 12000;
//		fruPrices[3] = 5000; 에러 발생 배열의 크기 보다 
//		                     더 많은 index 호출
		...
		System.out.println("몸무게 배열의 갯수:"+weights.length);
		// 배열과 mapping된 for문 처리
		// int idx=0 : 배열의 시작 index
		// idx<weights.length : 배열의 한계치 범위 
		//                  index는 크기보다 1작다
		// idx++ : 배열의 증가 처리
		// weights[idx] : 배열의 구성요소 접근 처리
		for(int idx=0;idx<weights.length;idx++) {
			System.out.print(idx+1+"번째 회원 몸무게 ");
			System.out.println(weights[idx]+"kg");
		}
		// ex) 회원3명의 이름과 회원의 나이를 배열로 선언 할당 후,
		//    반복문 for를 통해서 출력하세요..
		//    번호   회원명   회원나이
		//    1     @@@     @@
		//    2     @@@     @@
		//    3     @@@     @@
		String []mnames = {"홍길동","김길동","신길동"};
		int []mages = {21,25,30};
		...
		// ex1) 위 mages를 for each구문을 통해 출력하고,
		//      전역변수를 활용하여 평균 나이를 출력하세요..
		// ex2) [알고리즘 문제]
		//      전역변수 추가하고, 최대 나이를 출력하세요.
		int tot = 0;
		int maxAge = 0;
		for(int age:mages) {
			tot+=age;
		...
		System.out.println("나이의 평균:"+(double)tot/mages.length);
