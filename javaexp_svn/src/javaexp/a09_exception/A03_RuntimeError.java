...
public class A03_RuntimeError {
	...
	public static void main(String[] args) {
		...
		1. try문을 반복문 안에 넣는 경우..
			해당 step에서 예외처리를 하고 다음 step으로 넘어갈 수
			있다.
			ex)
			for(int cnt=10;cnt>=-10;cnt--) {
				try{
		...
		try {
			String s=null;
			s.toString(); 
			// 객체속성 호출.. 즉, 객체가 heap영역이 생성되지 않은
			// 상태에서 메서드를 호출하니, 주소값이 없습니다
			// 에러가 나오는데, 이것을 처리해주는 예외 처리내용..
			// NullPointerException : 
			// Pointer : 주소가 Null 없을 때, Exception에러..
...
1. 프로그래밍을 하면, 에러나 여러가지 오류로 인하여 처리가 문제가
	...
	치명적으로 영향을 미치기에 보다 효과적인 대응하는 것이 필요하다.
...
3. 오류의 종류
	...
	3) 프로그래밍 처리 방법으로 인한 구분
		- 일반(컴파일 체크)예외
			...
			컴파일도 안되기에 툴에서는 왼쪽에 빨간색으로 표기 된다.
			==> 필수 예외 처리가 필요한 것으로 IO(input output)
			처리되는 것과 메모리가 로딩되는 것등이 주로 이 예외 처리
			에러이다. ex) IO(파일읽기/쓰기/데이터 전송), DB처리,
			네트워킹 처리 등
		- 실행(런타임 에러)예외
			예외 처리 코드를 생략하더라도 컴파일이 되는 예외로
			경험에 따라 예외 처리 코드 작성이 필요한다.
			실행 후, 실행되어 처리되는 데이터에 대한 예외를 발생하는
			경우 ==> heap메모리 호출에러(NullPointerException),
			배열에러(IndexOutOfBoundsException), 0나누기
			(ArithMethicsException)등 프로그램에 흔하게 발생하는
			에러 처리이다.
4. 단계별 예외 처리 형식
	1) 1단계 기본형식
		try{
			...
			예외가 나왔을 때, 처리할 내용
			e.기능메서드() : 예외에 대한 정보를 처리할 수 있는 내용
				수행
	...
	3) 3단계 예외의 위침
		...
		모든 곳에 메서드별로 처리해야 하는데, 이것을 기능메서드별로 
		구분하여 처리하지 않고, 이 메서드가 호출할 때, 처리하는 것을
		예외 위임이라고 한다.
		- 코드의 중복성을 줄이고 
		try{}catch(){}블거 각 메서드별로 try{}catch(){}블럭
		사용하지 않아도 된다.
		- 체계적으로 분류하여 예외 처리를 관리하기 위한 목적
			다중 예외 처리를 통해 효과적으로 관리할 수 있다.
			ex) NullPointer처리할 예외 블럭 내용 기술
			수학관련 처리할 예외 블럭 내용 기술
			배열관련 처리할 예외 블럭 내용 기술
			컴파일관려 처리할 예외 블럭 기술
		메소드별로 예외를 위임하여 처리 : 2개 이상도 예외를 위임할 
			수 있다.
