...
public class A01_MVC_StartController {
	...
	public String call10(@RequestParam(value = "name", defaultValue = "") String name,
	...
	public String call11(Model d) {
		...
		/*
		# controller 단에서 위와 같이 설정된 모델 데이터를, view 단에서 아래와 같이 호출할수 있다.
		${name}
		${age+10}
		...
		price 과일의 가격 선언과 할당
		Movie Vo 를 이용해서 상영일 영화제목 동반자 수 를 할당하여 화면에서 출력
		*/
		d.addAttribute("m1", new Movie("2023-12-22", "괴물", 5));
		d.addAttribute("fruit", "수박");
		d.addAttribute("price", 10000);
