...
<div class="container">
	<form id="frm01" class="form"  method="post">
  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	    <input placeholder="검색할 이름" name="nameSch" value="${param.nameSch}"  class="form-control mr-sm-2" />
	    <input placeholder="검색할 권한" name="authSch" value="${param.authSch}"   class="form-control mr-sm-2"/>
   ...
   <table class="table table-hover table-striped">
    ...
    <c:if test="${not empty mem.id}"><%-- 등록전 초기화면 구분 --%>
    	<script>
    		var regCnt = "${dao.insertMember(mem)}"
    		alert(regCnt=="0"?"등록실패":"등록성공")
    	</script>
    </c:if> 
    <tbody>
    	<c:forEach var="m" 
    		items="${dao.getMemberList(param.nameSch, param.authSch)}">
    	<tr><td>${m.mno}</td>
    		<td>${m.name}</td>
    		<td>${m.id}</td>
    		<td>${m.pwd}</td>
    		<td>${m.auth}</td>
    		<td><fmt:formatNumber value="${m.point}"/>
    		</td></tr>
    	</c:forEach>	
    </tbody>
	</table>    
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
		<form id="frm02" class="form"  method="post">
	     <div class="row">
	      <div class="col">
	        <input type="text" value="0" readOnly class="form-control" placeholder="회원번호 입력" name="mno">
	      </div>
	      <div class="col">
