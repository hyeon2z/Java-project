...
1. 부서정보 상세 로딩
	...
	2) 삭제 처리.
		프로세스 : 삭제 버튼을 클릭시, ajax로 요청값을 해당 삭제 jsp에 deptno를 전달하고
		해당 삭제.jsp에서 dao를 호출하여 삭제메서드를 통해서 삭제가 되게 처리한다.
		...
		처리 순서
			삭제 JSP(z16_deptDel.jsp)
				useBean으로 dao 선언
				<jsp:useBean id="dao" 
					class="backendweb.d01_dao.PreparedStmtDao"/>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#regFrmBtn").click(function(){
			...
			$("#frm02 [name=deptno]").attr("readonly",false)
			$("#frm02")[0].reset()
			$("#regBtn").show();
			$("#uptBtn").hide();
			$("#delBtn").hide();
