...
<%--
# response 기본 객체
1. 웹브라우저에 전송하는 응답 정보를 설정하는 객체
2. 주요 기능
	1) 헤더 정보 입력
	2) 리다이렉트 처리 (서버단 안에서 페이지이동처리)
3. 기능 메서드
	...
	4) setDateHeader(String name, long date)
		name헤더의 값을 date로 지정한다. date는 1970/1/1 이후 흘러간
...
// a09_login.jsp
// 아이디/패드워드 입력 
// ==> himan/7777일때는 a10_main.jsp 로그인성공 메인페이지
//     그외             a11_logfailure.jsp 로그인실패 표시
//                     로그인페이지이동 클릭시, a09_login.jsp 이동처리
/*
# 진행 순서
1. 페이지 만들기( a09_login.jsp, a10_main.jsp,  a11_logfailure.jsp)
2. form 하위에 name="id" name="pwd"  설정하기
3. 요청값 만들기 id/pwd
4. 요청값 if(id==null) id=""; 형식으로 null 처리하기
5. if(id.equals("himan") && pwd.equals("7777")){
		response.sendRedirect("a10_main.jsp");
   }else{
	   response.sendRedirect("a11_logfailure.jsp");
   }
6. a11_logfailure.jsp
	<h1 onclick="location.href='a09_login.jsp'">로그인페이지이동</h2>
	추가..
...
<div class="container">
   ...
   <table class="table table-hover table-striped">
    ...
    <tbody>
    	...
    	// ==> request로 처리 
    	// 다시 이 헤더 정보를 변경할려면 response객체를 통해서
    	// 속성을 변경하여 전달==> 위 나열된 response.XXXX()메서드를 통해서
    	// 처리...
