...
3. 속성
	type: 포맷할 숫자의 타입을 지정한다.
		속성값으로는 number(일반숫자), currency(통화), percent(%표시)
	pattern: 사용자 정의 숫자 포맷을 지정한다..
		- 속성값으로 설정할 수 있는 포맷형식
			"###,###.##" :천단위 구분 기호를 사용하여 숫자를 포맷한다.
				소숫점이하 두자리까지 표현
			"000.0000" : 숫자 앞에 빈자리가 있을 경우 0으로 채우고,
				소숫점 이한 네 자리까지 표시한다
   ...
   <table class="table table-hover table-striped">
    ...
    <tbody>
    	<tr>
    		...
    		<td><fmt:formatNumber value="${price}" type="number" 
    				var="numberType"/>변수:${numberType} </td>
    		<%-- 날짜데이터가 지정된 형식을 변수를 할당하고 그 변수를 출력하고 활용
    		     할 때 사용된다. --%>		
    		<td><fmt:formatNumber value="${price}" type="currency"
    				currencySymbol="원" /> </td>
    		<td><fmt:formatNumber value="${price}" type="percent"/></td>
    		<td><fmt:formatNumber value="${price}" pattern="0000000.00"/>
    		<td><fmt:formatNumber value="${price}" pattern="###,###.#원"/>
