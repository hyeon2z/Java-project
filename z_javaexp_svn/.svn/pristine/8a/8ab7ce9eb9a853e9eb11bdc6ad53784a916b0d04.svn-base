	...
	public static void main(String[] args) {
		...
		3. 형태
			...
			3) 추가 선언하는 생성자
				...
				비롯하여 매개변수 오버로딩 규칙(갯수, 타입,순서)으로 
				여러 생성자를 선언 할 수 있다.
			4) 생성자의 매개변수는 일반적으로 필드를 초기화하는 경우가
		...
		Product p03 = new Product("딸기",12000,5);
		...
		Dog d1 = new Dog();  
		// 생성자가 선언하지 않을 때는 default 생성자가 컴파일시 자동
		// 생성되어 사용할 수 있다.
...
//		선언된 생성자가 있으면 사라진다.
		...
		Phone p1 = new Phone();
		Phone p2 = new Phone("삼성전자");
		Phone p3 = new Phone(1000000);
		Phone p4 = new Phone("삼성전자",1000000);
		Phone p5 = new Phone(1000000,"LG전자");
...
// ex) Product 물건명/가격/갯수를 필드로 선언하고, 생성자를 통해서
