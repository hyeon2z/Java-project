...
2. 테이블 생성 단계별 학습
	...
	2) 데이터 유형
		number :  숫자형 유형
			number : 정수와 실수가 모두 입력되는 기본
				유형의 숫자형 타입
			number(전체자리수, 소수점이하자리수)
		varchar2(byte) : 가변형 문자열
			최대크기를 설정하여, 그 이하의 데이터를 
			입력할 수 있다.
			varchar2(10)
				최대문자열 10bytes
				    10byte이하의 문자열을 저장하고,
				    해당 크기만큼 메모리를 사용한다.
				    'himan' 할당된 경우 5byte 메모리를
				    이용하여 할당이 된다.
				    '안녕하세요' 3x5 = 15byte 할당자체가
				    되지 않는다. 최대값을 넘었기 때문.. 
				영문은 글자 10, 한글은 오라클에서는
				3byte가 한문자이기에 3자 입력 가능하다.
		char(byte) : 고정형문자열
			해당 설정한 데이터 크기로만 메모리를 할당하는 데이터
			유형이다. 
			char(10) : 최대크기보다 작은 데이터가 입력가능하나
				메모리는 10byte가 할애되고 나머지는 공백이
				할당된다.
		date : 날짜형 유형
			1/1000 초부터 초분시 일월년도 등과 같이 날짜
	        관련 데이터를 저장한다.
	        ps) sysdate라는 내장형 날짜 데이터를 통해서
	        오늘 현재시간의 날짜형 데이터를 입력할 수 있다.
	    ps) 오라클 clob, file 타입을 통해서 binary로 파일을
	    	stream형식으로 전달해서 담을 수 있는지만, 
	    	실무적으로 너무 낭비인 경우가 많다.
	    	일반적으로 파일서버를 따로 만들고, 해당 실제 파일을
	    	해당 파일서버에 담고, 파일을 경로/파일명을 oracle의 varchar2
	    	로 저장한다.
...
SELECT * FROM dept;
-- ex) dept01(전체복사), dept02(구조만)  테이블 만드세요.
CREATE TABLE dept01
AS SELECT * FROM dept;
CREATE TABLE dept02
