...
2. 복수행 함수 : 조건에 따라 여러 행을 그룹화하여 그룹별로 결과를 하나씩 반환하는 함수
	1) 사용방법
		...
		select max(hiredate)
		from emp;
		-- 사원의 입사일 중에 가장 높은 것으로 최근에 입사한 일사일을 출력
...
FROM emp;
-- ex) || 연결연산자를 활용하여
--     사원명 is a 직책명으로 출력하세요 ex) Smith is a Clerk!
SELECT INITCAP(ename)||' is a '||INITCAP(job)||'!' show
FROM emp;
/* 2. 대소문자 변환 함수
 * 	  주로 실무상 코드명이나 물건명을 영문으로 되어 있는 경우 대소문자 구분없이
...
FROM EMPLOYEES e;
-- ex) employees 테이블에서 job_id는 소문자로, email도 소문자로
--   first_name은 대문자로 변환하여 || 연결연산자와 혼합하여 
--   직책 @@@(job_id)인 @@@(first_name)의 이메일은 @@@(email) 입니다. 출력
SELECT '직책 '||LOWER(job_id)||'인 '||UPPER(first_name)||'의 이메일은'|| 
		lower(email)||' 입니다. ' show
FROM EMPLOYEES e;

SELECT * FROM EMPLOYEES;
-- first_name 컬럼을 기준으로 do, DO, dO 입력값 상관없이 Do라는 글자가
-- 대소문자 관련 없이 검색이 되게 할려면 다음과 같이 처리하여야 한다.
SELECT first_name, employee_id, UPPER(first_name)
FROM EMPLOYEES e 
WHERE UPPER(first_name) LIKE '%'||upper('do') ||'%';
--WHERE lower(first_name) LIKE '%'||lower('do') ||'%';
--  첫번째이름[do  ] 마지막이름[   ]  [검색] ==> 검색시,
--      입력값의 대소문자 상관없이 검색을 처리할 때, 사용할 sql 처리 부분(keyword)

-- upper('do') 부분에 어떤 데이터를 입력하더라도 대문자로 바꾸고,
-- 다시 컬럼을 upper(컬럼명)에 의해 대문자로 변환된 내용을 검색되게 한다.
-- ex) employee테이블에 email와 job_id가 대소문자 관련없이 키워드 검색되
--    게하세요.  연습 키워드 'o', 'A'
