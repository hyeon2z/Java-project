...
에 대한 특정 권한을 가질 수 있게 하여, 다수의 사용자가 데이터베이스에
...
3. 권한의 역할과 종류
	...
	6) create session : 데이터베이스에 접속할 수 잇는 권한
...
2. 전체 oracle에 사용되는 여러 객체(테이블, index, sequence 등등)에 대한 
	권한 부여 
	grant resource to 계정명
	ex) grant resource to user01;
3. 전체 계정에 대한 권한 부여 내용 확인 하는 테이블(시스템 테이블)
	dba_users
...
4. 테이블(물리적으로 저장이 필요한 객체) tablespace라는 저장공간을 할당
	...
	alter user 사용자명 quota unlimited on users;
		사용자가 사용할 테이블 스페이스의 용량을 지정..
		
ex) user03//9999  계정과 비번을 만들고, 접속과 자원에 대한 접근권한을 
	주고, 테이블을 생성하고 데이터를 입력하세요.
 */
select * from dba_users;
