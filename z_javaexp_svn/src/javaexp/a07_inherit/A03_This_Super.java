...
class Person{
	...
	Person(String name, int age, String loc, String str){
		this.name = name;
		this.age = age;
		//this.loc = loc;
		// 현재 정의된 생성자 중에 이름과 나이가 있는 생성자 호출 
...
class Man extends Person{
	/* default로 들어가 있는데, 상위에 매개변수가 없는 생성자가
	 * 없는 순간 반드시 호출하여야 한다.
	public Man(){
		super();
	}
	...
	public Man(String name) {
		// 상위에 여러 개의 매개변수 생성자 중에 추가하여 선택적으로 호출할 수 있다.
		//super()
		super(name);
	...
	public Man(String name, int age) {
		...
		super(name, age); 
		// super, this 생성자 호출 첫라인에서 호출하여야 한다.
	}
...
class Product02{
	...
	Product02(String name, int price, int cnt){
		this(name,price);
		this.cnt = cnt;
	}
}
