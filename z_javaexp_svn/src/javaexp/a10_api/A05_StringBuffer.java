...
public class A05_StringBuffer {
	...
	public static void main(String[] args) {
		...
		/*
		# String 메모리 +=의 위험성..
		1. String은 문자열 데이터인데 +=을 통해서 문자열 추가할 
			수 있는데, 기존에 특정한 위치에서 메모리가 확대되는 것이
			아니라, 새로운 주소값을 가지고 다시 메모리를 할당하는
			특징이 있다.
		2. 반복문안에 +=을 사용하여 문자열을 추가하는 것을 메모리의
			효율에 문제가 발생한다.
		...
		통하여 데이터가 누적하당하면 heap여역을 계속 만들어 메모리의
		overflow가 발생할 수 있다.
		...
		4. 기타 기능 메서드
			...
			reverse() :문자열 뒤집기 처리
			setCharAt(int idx, char ch) : 특정 위치에 문자를
				대체처리(문자 한자)
		 * */
		...
		sb.append("홍");System.out.println("추가후 주소값:"+
		...
		StringBuffer sbf2 = new StringBuffer(1+"번, ");
		System.out.println("초기 주소값:"+
				System.identityHashCode(sbf2));
		System.out.println(sbf2);
		for(int cnt=2;cnt<=100;cnt++) {
			sbf2.append(cnt+"번, ");
		}
		System.out.println("최종 주소값:"+
				System.identityHashCode(sbf2));
		System.out.println(sbf2);		
