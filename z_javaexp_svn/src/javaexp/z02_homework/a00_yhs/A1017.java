...
`main()`에서 이러한 사용자 정의 예외를 발생시키려면 `throw` 키워드를 사용하며, 예외를 처리하려면 `try-catch` 블록을 사용해야 합니다.
...
String str1 = "안녕"
String str2 = "안녕"
"안녕" 이라고 사용하는 곳은 heap영역에 같은 곳을 바라 본다.
String str3 = new String("안녕");
String str4 = new String("안녕");
str3, str4는 다른 heap주소를 가진다.
...
public class PasswordGenerator {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            passwordBytes[i] = (byte) (Math.random()*26 + 'a');
        }
...
`toString()` 메서드는 객체의 문자열 표현을 반환하는데 사용됩니다. 기본적으로 `클래스명@해시코드` 형태의 문자열을 반환하지만, 사용자 정의 클래스에서 `toString()`을 재정의하면 원하는 형태로 문자열 표현을 반환할 수 있습니다.
